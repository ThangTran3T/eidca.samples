/**
 * Step3Check.js — STEP 3: Kiểm tra trạng thái cấp CTS (Polling)
 *
 * Gọi API: POST /ca/api/eid-personal/check (lặp lại mỗi interval ms)
 *
 * Input từ state:
 *   - transactionCode  — từ STEP 1/2
 *   - tokenSignature   — từ STEP 2
 *   - interval         — ms, từ response STEP 2 (mặc định 3000ms)
 *
 * Logic poll:
 *   - Gọi /check mỗi interval ms
...
import { personalCheckStatus } from "../../api/eidcaClient.js";
...
export class Step3Check {
  ...
  mount() {
    ...
    this._el.innerHTML = `
      <div class="step-card-header">
        ...
        <div class="step-card-title">
          ...
          <p>POST /ca/api/eid-personal/check — Kiểm tra trạng thái định kỳ</p>
        </div>
      ...
      <div class="step-card-body">
        ...
        <div class="poll-progress hidden" id="poll-progress">
          ...
          <div class="text-muted text-sm" id="poll-interval-label" style="margin-top:6px;">
            Kiểm tra lại mỗi <span id="poll-interval-val">3</span>s
          </div>
        </div>

        <!-- Kết quả CTS (hiện sau khi completed) -->
        ...
        <!-- Code Panels: Request | Response (cập nhật mỗi lần poll) -->
    ...
    this._codePanels = renderCodePanels(
      ...
      { requestTitle: "POST /ca/api/eid-personal/check" }
    );
  ...
  async _poll() {
    ...
    try {
      // ── Gọi API ──────────────────────────────────────────────────────────
      // personalCheckStatus() → POST /ca/api/eid-personal/check
      const data = await personalCheckStatus(
    ...
    section.innerHTML = `
      <div class="cts-result">
        <div class="cts-header">
          ...
          <div>
            <div class="cts-title">Đăng ký Chứng thư số thành công!</div>
            <div class="cts-sub">CTS đã được cấp và kích hoạt</div>
          </div>
        </div>
